<?php
    ...
    if ($islem == "sil") {
        $id = $_GET["id"];
        $sorgu = $baglan->query("delete from tour_11 where (id='$id')");
        echo "<script> window.location.href='tour_11.php'; </script>";
    }
    ...
    if ($islem == "ekle") {
        ...
        $text = $_POST["text"];
        $sorgu = $baglan->query("insert into tour_11 (name,image,text) values ('$name','../$image','$text')");
        echo "<script> window.location.href='tour_11.php'; </script>";
    }
?>
<!DOCTYPE html>
<html>
    <head>
        ...
        <?php
        $sirano = 0;
        $sorgu = $baglan->query("select * from tour_11");
    ...
    <div style="text-align:center;">
        ...
        while ($satir = $sorgu->fetch_object()) {
            ...
            echo "<tr>
                    ...
                    <td>$satir->image</td>
                    <td>$satir->text</td>
                    <td><a href='tour_11.php?islem=sil&id=$satir->id'>DELETE</td>
                </tr>";}
        ?>
        </table><br>
        <form action="tour_11.php?islem=ekle" enctype="multipart/form-data" method="post">
