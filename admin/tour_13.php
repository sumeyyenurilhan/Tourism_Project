<?php
    ...
    if ($islem == "sil") {
        $id = $_GET["id"];
        $sorgu = $baglan->query("delete from tour_13 where (id='$id')");
        echo "<script> window.location.href='tour_13.php'; </script>";
    }
    ...
    if ($islem == "ekle") {
        ...
        $text = $_POST["text"];
        $sorgu = $baglan->query("insert into tour_13 (name,image,text) values ('$name','../$image','$text')");
        echo "<script> window.location.href='tour_13.php'; </script>";
    }
?>
<!DOCTYPE html>
<html>
    <head>
        ...
        <?php
        $sirano = 0;
        $sorgu = $baglan->query("select * from tour_13");
    ...
    <div style="text-align:center;">
        ...
        while ($satir = $sorgu->fetch_object()) {
            ...
            echo "<tr>
                    ...
                    <td>$satir->image</td>
                    <td>$satir->text</td>
                    <td><a href='tour_13.php?islem=sil&id=$satir->id'>DELETE</td>
                </tr>";}
        ?>
        </table><br>
        <form action="tour_13.php?islem=ekle" enctype="multipart/form-data" method="post">
